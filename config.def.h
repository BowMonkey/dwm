...
static const char *colors[][3]{
    ...
    /*SchemeBarEmpty  空白状态栏*/   {col_borderbar,col_borderbar, 0x00 },
    /*SchemeOverView  overview  */   { red2, black, black },
};
//-----------------------------------------------------------------------------
static int statusbar_h_bias=15;
static int tag_line_h=4; //under line of selected tag
//-----------------------------------------------------------------------------
static const unsigned char borderalpha = 0xdd;      /* 边框透明度 */
// 透明度设置
static const unsigned int alphas[][3] = {
    // 不同的可选项：0x00 0xab QPAQUE borderalpha
    // 千万不可以修改顺序，只可以在当前位置改颜色，修改顺序就乱了
    /*  ColFg, ColBg, ColBorder */
    /*SchemeNorm      普通      */   { 0x00, 0xff, borderalpha },
    /*SchemeSel       选中      */   { 0x00, 0xff, borderalpha },
    /*SchemeSelGlobal 全局选中  */   { 0x00, 0xff, borderalpha },
    /*SchemeTabSel    选中tag   */   { 0x00, 0xff, borderalpha },
    /*SchemeTabNorm   普通tag   */   { 0x00, 0xff, borderalpha },
    /*SchemeUnderline 下划线    */   { 0xff, 0xff, borderalpha },
    /*SchemeMode      模式      */   { 0x00, 0xff, borderalpha },
    /*SchemeHid       隐藏      */   { 0x00, 0xff, borderalpha },
    /*SchemeSystray   托盘      */   { 0x00, 0xff, borderalpha },
    /*SchemeNormTag   普通标签  */   { 0x00, 0xff, 0x00 },
    /*SchemeSelTag    选中标签  */   { 0x00, 0xff, 0x00 },
    /*SchemeBarEmpty  空白状态栏*/   { 0xff, 0xff, 0x00 },
    /*SchemeOverView  overview  */   { 0xff, 0xff, borderalpha },
    /*SchemeStatusText状态栏文本*/   { 0x00, 0xff, 0x00 },
};
...
//-----------------------------------------------------------------------------
//                     状态栏启动脚本
static const char *statusbarscript = "~/Desktop/dwm/statusbar/statusbar.py";//coco
...
static const Rule rules[] = {
    ...
    // {"wemeetapp",   0x00,       0x00,      TAGMASK,     1,         1,        0,        -1}, // 腾讯会议有bug,需要使用global类型
    {"copyq",       0x00,       0x00,           0,     1,         1,        0,        -1},
    {"Nitrogen",    0x00,       0x00,           0,     1,         0,        0,        -1},
    {0x00,          0x00,       "QQ",           0,     1,         0,        0,        -1},  //
    {0x00,          0x00,       "WeChat",       0,     1,         0,        0,        -1},  //

    //-----------------------------------------------------------------------------
    // 以下适用于无边框，浮动窗口，全局显示，请勿更改
    {"FG", 0x00, 0x00, 0, 1, 1, 1, -1},      // 浮动 + 全局
    {"FN", 0x00, 0x00, 0, 1, 0, 1, -1},            // 浮动 + 无边框
    {"GN", 0x00, 0x00, 0, 0, 1, 1, -1},      // 全局 + 无边框
    {"FGN", 0x00, 0x00, 0, 1, 1, 1, -1},     // 浮动 + 全局 + 无边框
    {"float", 0x00, 0x00, 0, 1, 0, 0, -1},         // 浮动
    {"noborder", 0x00, 0x00, 0, 0, 0, 1, -1},      // 无边框
    {"global", 0x00, 0x00, 0, 1, 1, 0, -1},  // 全局
};
...
static const Layout layouts[] = {
    ...
    { "﬿",          flextile,         { -1, -1, SPLIT_VERTICAL, TOP_TO_BOTTOM, TOP_TO_BOTTOM, 0, 0x00 } }, // default tile layout
    { "﩯",        magicgrid,{0} },    /* 网格 */
    { "|||",      flextile,         { -1, -1, SPLIT_VERTICAL, LEFT_TO_RIGHT, TOP_TO_BOTTOM, 0, 0x00 } }, // columns (col) layout //其实就是tile
    { ">M>",      flextile,         { -1, -1, FLOATING_MASTER, LEFT_TO_RIGHT, LEFT_TO_RIGHT, 0, 0x00 } }, // floating master
    { "TTT",      flextile,         { -1, -1, SPLIT_HORIZONTAL, LEFT_TO_RIGHT, LEFT_TO_RIGHT, 0, 0x00 } }, // bstack
    { "===",      flextile,         { -1, -1, SPLIT_HORIZONTAL, LEFT_TO_RIGHT, TOP_TO_BOTTOM, 0, 0x00 } }, // bstackhoriz
    { ":::",      flextile,         { -1, -1, SPLIT_HORIZONTAL, LEFT_TO_RIGHT, TOP_TO_BOTTOM, 0, monoclesymbols } }, // centeredmaster
    { "gapless",  flextile,         { -1, -1, NO_SPLIT, GAPPLESSGRID, GAPPLESSGRID, 0, 0x00 } }, // gappless grid
    { "(@)",      flextile,         { -1, -1, NO_SPLIT, SPIRAL, SPIRAL, 0, 0x00 } }, // fibonacci spiral
    { 0x00,       0x00,             {0} },//最后一个需要是0x00,0x00,cyclelayout,请勿更改
};
...
//                          不需要更改
//=============================================================================
#define SHCMD(cmd) { .v = (const char*[]){ "/bin/sh", "-c", cmd, 0x00 } }
...
static Key keys[] = {
    ...
    //-----------------------------------------------------------------------------
    { MODKEY|ShiftMask,    XK_h,       ExchangeClient,    {.i = 0} },   // 移动窗口  in layout
    { MODKEY|ShiftMask,    XK_j,       ExchangeClient,    {.i = 1 } },  // 移动窗口  in layout
    ...
    { MODKEY|ControlMask,  XK_q,       killtag,           {.i = 0} },  // kill current tag
    //-----------------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------------
    { MODKEY,              XK_Up,      movewin,           {.ui = UP} },   // 移动窗口  floating window
    { MODKEY,              XK_Down,    movewin,           {.ui = DOWN} }, // 移动窗口  floating window
    { MODKEY,              XK_Left,    movewin,           {.ui = LEFT} }, // 移动窗口  floating window
    { MODKEY,              XK_Right,   movewin,           {.ui = RIGHT} },// 移动窗口  floating window
    { MODKEY|ControlMask,  XK_Up,      resizewin,         {.ui = V_REDUCE} },// 调整窗口 floating window
    { MODKEY|ControlMask,  XK_Down,    resizewin,         {.ui = V_EXPAND} },// 调整窗口 floating window
    { MODKEY|ControlMask,  XK_Left,    resizewin,         {.ui = H_REDUCE} },// 调整窗口 floating window
    { MODKEY|ControlMask,  XK_Right,   resizewin,         {.ui = H_EXPAND} },// 调整窗口 floating window
    //-----------------------------------------------------------------------------
    ...
    //                              多显示器配置
    //=============================================================================
    //{ MODKEY,              XK_Left,     focusmon,         {.i = -1} },     // 光标移动到另一个显示器
    //{ MODKEY,              XK_Right,    focusmon,         {.i = +1} },     // 光标移动到另一个显示器
    //{ MODKEY,              XK_h,        focusmon,         {.i = -1} },     // 光标移动到另一个显示器
    //{ MODKEY,              XK_l,        focusmon,         {.i = +1} },     // 光标移动到另一个显示器
    ...
    TAGKEYS(XK_8, 7,  0)
    TAGKEYS(XK_9, 8,  0)
    TAGKEYS(XK_c, 6,  "/usr/bin/firefox")
    TAGKEYS(XK_m, 7,  "/usr/bin/lx-music-desktop")
    //-----------------------------------------------------------------------------
